    ...
    >
      <div>
        <div className="flex items-center px-2 h-16 gap-3 border-b border-white/5">
          ...
          <div className={`flex flex-col leading-tight transition-all duration-300 ${open ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
             <span className="text-white font-bold text-xl uppercase whitespace-nowrap">Sena<span className="text-orange-500">i</span></span>
        ...
        <nav className="mt-6 flex flex-col gap-1 px-3">
          <MenuItem icon={<RiHomeLine />} onClick={() => navigate("/homeadmin")} label="Início" open={open} active={isActive("/")} />
          <MenuItem icon={<GoGear />} onClick={() => navigate("/config")} label="Configurações" open={open} active={isActive("/config")} />
        </nav>
      </div>

      <div className="p-3 mb-4 flex flex-col gap-1 border-t border-white/5">
        <MenuItem icon={darkMode ? <PiSunHorizonDuotone /> : <PiMoonStarsDuotone />} label={darkMode ? "Claro" : "Escuro"} open={open} onClick={() => setDarkMode(!darkMode)} />
        <MenuItem icon={<IoLogInOutline />} onClick={() => navigate("/login")} label="Sair" open={open} active={isActive("/login")} />
      </div>
    </aside>
...
function MenuItem({ icon, label, open, onClick, active }) {
  return (
    <button
      onClick={onClick}
      className={`
        relative group flex items-center w-full gap-4 px-3 pl-4 py-3 rounded-xl
